...
time = d1['time'][:]
sst = d1['sst'][(time.dt.year>=1901)&(time.dt.year<=2022)][:,0,:,:]
sst = np.array(sst)
    ...
    for l in range(10):
        for m in range(50):#5N-5S, 170W-120W
            if not np.isnan(sst[i,85+l,m+190]):
                ...
                co2+=1
    nino[i]=nino[i]/co2
nino34 = np.zeros((1464))    
for i in range(2,1462):
    nino34[i]=np.mean(nino[i-2:i+3],axis=0)
nino34[0] = np.mean(nino[0:3],axis=0)
nino34[1] = np.mean(nino[0:4],axis=0)
nino34[1462] = np.mean(nino[1460:1463],axis=0)
nino34[1463] = np.mean(nino[1460:],axis=0)
nino7 = nino34[6::12]
nino8 = nino34[7::12]
index = (nino7+nino8)/2  
