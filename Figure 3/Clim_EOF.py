...
for k in range(4):
   for i in range(30):
       for j in range(30):
           if not (np.isnan(pre[k][:,i,j]).any()):
               scipy.signal.detrend(pre[k][:,i,j], axis=0, type='linear', bp=0, overwrite_data=True)
...
for i in range(6):
    ...
    shp_path1 = r'/Users/fuzhenghang/Documents/python/Pakistan/national/national.shp'
    reader = Reader(shp_path1)
    nafeat=cfeature.ShapelyFeature(reader.geometries(), ccrs.PlateCarree(), edgecolor='k', facecolor='none',zorder=2)
    ax[i].add_feature(nafeat, linewidth=0.35)
    ...
    ax[i].text(-122,36,tit2[i],fontweight='bold',fontsize = 10)
    if i not in [0,3]:
...
corr1=df1.corr()
print(corr1)
plt.savefig('/Users/fuzhenghang/Documents/python/Pakistan/figs/fig3.eps', format="eps", bbox_inches = 'tight',pad_inches=0,dpi=600)
