...
dx = 0.45
dy = 0.22
loc = [[0.46, 0.77, 0.0085, 0.22],[0.457, 0.78, 0.0085, 0.2],[0.535, 0, 0.18, 0.012],[0.742, 0, 0.18, 0.012]]
...
for i in range(2):
    if i not in [1]:
        ...
        ax[i].add_feature(cfeature.COASTLINE.with_scale('50m'),lw=0.5,color='k',zorder=5)
        ax[i].add_feature(cfeature.OCEAN.with_scale('110m'),color='w',lw=0,zorder=2)
        ...
        if i == 0:
            CS=ax[i].contour(M.lo[i],M.la[i],-1*M.pattern[0],[-0.1+0.02*k for k in range(11)], cmap = cmaps.MPL_BrBG_r ,linewidths = 1.8,transform=ccrs.PlateCarree(),extend='both',zorder=2)
            ...
            ax[i].text(57.8,46.5,'A',fontsize=11,fontweight='bold')
        ...
        ax[i].axhline(y=0,  linestyle='-',linewidth = 0.6,color='black',alpha=1,zorder=1)
        if i == 1:
            ax[i].text(1977,4.55,'B',fontsize=11,fontweight='bold')
            ax[i].yaxis.tick_left()
            ax[i].yaxis.set_label_position("left")
            y1 = np.zeros((46))-0.6
            y2 = y1+1.2
            x1 = [1978+i for i in range(46)]
            ax[i].fill_between(x1, y1, y2, color='gainsboro',linewidths=0,zorder=0)
...
plt.savefig('/Users/fuzhenghang/Documents/python/Pakistan/figs/fig4.eps', format="eps", bbox_inches = 'tight',pad_inches=0,dpi=600)
