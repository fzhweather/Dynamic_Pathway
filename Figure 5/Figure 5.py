...
label = ['A','B']
sc = ['mm/month','day/year','gpm','Pa/s','gpm','$10^{-11}/s$']
label2 = ['C','D']
#cofi = cmaps.pengsa
#cofi = plt.cm.bwr
cofi = cmaps.ncaccept
...
for i in range(6):
    ...
    if i <= 1:
        ...
        ax[i].text(53,46,label[i],fontsize=12,fontweight='bold')
        if  i ==0:
            ...
            ax[i].text(53,-1,'E',fontsize=12,fontweight='bold')
    ...
    if i in [2,3]:
        ...
        ax[i].text(23,61,label2[i-2],fontsize=12,fontweight='bold')
        if  i == 2:
            gl.bottom_labels  = False
    ...
    if i == 5:
        ax[i].add_feature(cfeature.COASTLINE.with_scale('50m'),lw=0.6,color='k',zorder=2)
        ax[i].text(23,61.5,'F',fontsize=12,fontweight='bold')
    ...
    if i == 4:
        ...
        c = ax[i].contourf(V.x[i-4],V.level ,V.data[i-3],levels=np.arange(-200,200.001,20), extend = 'both',zorder=1, cmap=cofi)
        for k in range(25):
        ...
        cbar=plt.colorbar(c,fig.add_axes(loc[i]),orientation='vertical',ticks=[-200,-100,0,100,200],
                        aspect=20,shrink=0.2,pad=0.06)#方向 
        cbar.ax.tick_params(length=1.8,width=0.4,pad=1.5)
...
plt.savefig('/Users/fuzhenghang/Documents/python/Pakistan/figs/fig5.eps', format="eps", bbox_inches = 'tight',pad_inches=0,dpi=600)
