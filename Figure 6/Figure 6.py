...
for i in range(4):
    if i <= 1:
        ...
        ax[i].add_feature(cfeature.OCEAN.with_scale('110m'),zorder=2,color='w',lw=0)
        ax[i].add_feature(cfeature.COASTLINE.with_scale('50m'),lw=0.3,color='k',zorder=2)
        ax[i].text(56-180,56,tit1[i],fontsize=10,fontweight='bold')
        ...
        ax[i].add_feature(cfeature.COASTLINE.with_scale('50m'),lw=0.3,color='k',zorder=2)
        ax[i].text(-188,72,tit1[i],fontsize=10,fontweight='bold')
        ...
        for l1 in range(95,145,2):
            for l2 in range(60,150,2):
                if datap[i][l1,l2]<0.05 and data[i][l1,l2]<0:
                    ax[i].text(l2-180,l1-90,'.',fontsize=8,fontweight = 'bold',color = 'steelblue',zorder=1)
                if datap[i][l1,l2]<0.05 and data[i][l1,l2]>0:
                    ax[i].text(l2-180,l1-90,'.',fontsize=8,fontweight = 'bold',color = 'darkred',zorder=1)
...
plt.savefig('/Users/fuzhenghang/Documents/python/Pakistan/figs/fig6.eps', format="eps", bbox_inches = 'tight',pad_inches=0,dpi=600)
