...
time1 = d3['time'][:]   
tmean = d3['Z3'][(time1.dt.month>=7)&(time1.dt.month<=8)&(time1.dt.year>=1901)&(time1.dt.year<=2020)][:,0,:,:]
#print(tmean.shape)
...
for mm in range(10):
    ...
    #print(lons)
    time1 = d2['time'][:]   
    tas = d2['Z3'][(time1.dt.month>=7)&(time1.dt.month<=8)&(time1.dt.year>=1901)&(time1.dt.year<=2020)][:,mm,:,:]
    print(tas.shape)
    tas = tas - tmean
    ...
    preall[mm*114:(mm+1)*114] = scipy.signal.detrend(pre, axis=0, type='linear', bp=0, overwrite_data=True)
    hwall[mm*114:(mm+1)*114] = scipy.signal.detrend(hw, axis=0, type='linear', bp=0, overwrite_data=True)
    tasall[mm*114:(mm+1)*114,:,:] = scipy.signal.detrend(tas, axis=0, type='linear', bp=0, overwrite_data=True)
    tpall[mm*114:(mm+1)*114,:,:] = scipy.signal.detrend(tp, axis=0, type='linear', bp=0, overwrite_data=True)
    """
...
np.save("/Users/fuzhenghang/Documents/FudanU/大四上/热浪/中间数据/amipsprd_Reg_cesm10-z200.npy",reg)
np.save("/Users/fuzhenghang/Documents/FudanU/大四上/热浪/中间数据/amipsprd_R_cesm10-z200.npy",R)

reg = np.load("/Users/fuzhenghang/Documents/FudanU/大四上/热浪/中间数据/amipsprd_Reg_cesm10-z200.npy")
R = np.load("/Users/fuzhenghang/Documents/FudanU/大四上/热浪/中间数据/amipsprd_R_cesm10-z200.npy")
print(reg)
