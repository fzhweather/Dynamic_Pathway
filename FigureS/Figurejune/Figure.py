...
for i in range(2):
    if i <= 1:
        ...
        ax[i].add_feature(cfeature.COASTLINE.with_scale('50m'),lw=0.3,color='k',zorder=3)
        ...
        for l1 in range(95,145,2):
            for l2 in range(60,150,2):
                if datap[i][l1,l2]<0.05 and data[i][l1,l2]<0:
                    ax[i].text(l2-180,l1-90,'.',fontsize=8,fontweight = 'bold',color = 'navy',zorder=1)
                if datap[i][l1,l2]<0.05 and data[i][l1,l2]>0:
...
plt.savefig('/Users/fuzhenghang/Documents/python/Pakistan/figs/figs5.eps', format="eps", bbox_inches = 'tight',pad_inches=0,dpi=600)
