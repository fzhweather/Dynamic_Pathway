...
#print(lons)
time1 = d2['time'][:]   
tas = d2['TS'][(time1.dt.month>=6)&(time1.dt.month<=6)&(time1.dt.year>=1979)&(time1.dt.year<=2020)]
...
tp = d1['FLNT'][(time.dt.month>=6)&(time.dt.month<=6)&(time.dt.year>=1901)&(time.dt.year<=2020)][:,:,:]
...
pre = scipy.signal.detrend(pre1, axis=0, type='linear', bp=0, overwrite_data=True)
...
tp = scipy.signal.detrend(tp, axis=0, type='linear', bp=0, overwrite_data=True)
