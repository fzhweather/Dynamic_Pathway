...
# -*- coding: utf-8 -*-
"""
Created on Sat Aug 19 10:53:37 2023
...
for i in range(2):
    ...
    if i == 0:
        CS=ax[i].contour(MB.lo[i],MB.la[i],-1*MB.pattern[0],[-0.1+0.02*k for k in range(11)], cmap = cmaps.MPL_BrBG_r ,linewidths = 1.8,transform=ccrs.PlateCarree(),extend='both',zorder=1)
        ...
        ax[i].text(57.8,46.5,'A',fontsize=12,fontweight='bold')
        ...
        ax[i].axhline(y=0,  linestyle='-',linewidth = 0.6,color='black',alpha=1,zorder=2)
        
        if i == 1:
            ax[i].text(1977,4.55,'B',fontsize=12,fontweight='bold')
...
plt.savefig('/Users/fuzhenghang/Documents/python/Pakistan/figs/figs1.eps', format="eps", bbox_inches = 'tight',pad_inches=0,dpi=600)
