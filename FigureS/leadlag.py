    ...
    for j in range(360):
        for m in range(12):
            if not (np.isnan(sst[m::12,i,j]).any()):
                scipy.signal.detrend(sst[m::12,i,j], axis=0, type='linear', bp=0, overwrite_data=True)
...
hw = scipy.signal.detrend(hw, axis=0, type='linear', bp=0, overwrite_data=True)
...
hwobs = np.zeros((44))
preobs = np.zeros((122))
hw_raw= np.load("/Users/fuzhenghang/Documents/FudanU/大四上/热浪/中间数据/R_hw_days_1979-2022_monthly.npy")
...
for i in range(1):
    ...
    ax[i].axvspan(0.5, 2.5, color='gainsboro',lw=0)
...
plt.savefig('/Users/fuzhenghang/Documents/python/Pakistan/figs/figs3.eps', format="eps", bbox_inches = 'tight',pad_inches=0,dpi=600)
